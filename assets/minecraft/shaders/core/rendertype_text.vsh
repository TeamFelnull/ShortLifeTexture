...
void main() {
	...
	if(	Position.z == 0.0 && // check if the depth is correct (0 for gui texts)
		...
		) gl_Position = ProjMat * ModelViewMat * vec4(-ScreenSize + 100.0, 0.0, 0.0); // move the vertices offscreen, idk if this is a good solution for that but vec4(0.0) doesnt do the trick for everyone, 特定の解像度で謎の線が出るため-ScreenSizeへ変更
}
